...
export default {
    data() {
        return {
            ...
            targetKey : '',
            targetSort : '',
            form : { bizFilter : '0',  pageSize : 10, term : '', filterType : '1' },
            pagination : '',
            loading : true,
        }
    }, 
    ...
    methods : {
        ...
        sortUp : function(detail){
            ...
            if(indexL == "0"){
                this.$message("이전의 피처링된 기업이 없습니다.")
                return false
            }
            ...
            form.append("keyValue", nowFeaturingNo);
            form.append("sortValue", nowSortNo);
            form.append("targetKey", preFeaturingNo);
            form.append("targetSort", preSortNo);

            featuring.auditionUpdateSort(form)
                .then(data => {
                    if(data.status.code == "0"){
                        this.$message("피처링된 기업의 순서를 변경하였습니다")
                        this.fetchData()
                    }
                })
        },
        sortDown: function(detail){
            ...
            if(indexL == this.auditionList.length-1){
                this.$message("이후의 피처링된 기업이 없습니다")
                return false
            }
